...
async function handleHttpNewRequest(request,response){
    try{
        let newCar = {make: request.query.Make, model: request.query.Model,year: request.query.Year};
...
async function handleHttpShowRequest(request,response){
    try{
        let foundCar =  await model.getSingleCar(request.params.Make, request.params.Model, request.params.Year);
        response.status(200);
...
async function handleHttpShowAllRequest(request,response){
    ...
    if (err instanceof DatabaseError) {
      ...
      response.send({
        errorMessage:
          "Getting all the cars failed. Unexpected error occured" +
          err.message,
      });
    }
    }
...
async function handleHttpDeleteRequest(request,response){
    try{
        let deleteCar = {make: request.params.Make, model: request.params.Model,year: request.params.Year};
...
async function handleHttpUpdateMakeRequest(request,response){
    ...
    let updateCar = {make: request.params.Make, model: request.params.Model,year: request.params.Year};
    const {newMake} = request.body
    try{
        
        if(await model.updateCarMake(updateCar, newMake)){
            response.status(200);
            response.send({message:"The car's make has been successfully updated"});
        }
        else{
                response.status(500);
                response.send({
...
async function handleHttpUpdateModelRequest(request,response){
    ...
    try{
        ...
        if(await model.updateCarModel(updateCar, newModel)){
            response.status(200);
            response.send({message:"The car's model has been successfully updated"});
        }
        else{
                response.status(500);
                response.send({
...
async function handleHttpUpdateYearRequest(request,response){
    ...
    try{
        ...
        if(await model.updateCarYear(updateCar, newYear)){
            response.status(200);
            response.send({message:"The car's year has been successfully updated"});
        }
        else{
                response.status(500);
                response.send({
...
module.exports = {handleHttpNewRequest,handleHttpDeleteRequest,handleHttpShowAllRequest,handleHttpShowRequest,handleHttpUpdateMakeRequest,handleHttpUpdateModelRequest,handleHttpUpdateYearRequest,router,routeRoot}