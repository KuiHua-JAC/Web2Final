...
function validUser(email, password, firstName, lastName, userName, isAdmin) {
    ...
    else if(!firstName || !validator.isAlpha(firstName) || !lastName || !validator.isAlpha(lastName)) throw new InvalidInputError("The name cannot be empty and must only be letters");
    else if(!userName) throw new InvalidInputError("The username cannot be empty");
    else if(!typeof(isAdmin) == Boolean) throw new InvalidInputError("isAdmin needs to be a bool");
}
...
function isValidEmail(email) {
    if(!validator.isEmail(email)) throw new InvalidInputError("The email is not valid");
  }
...
function isValidPassword(password) {
    ...
    if (!(hasUppercase && hasLowercase && hasNumber)) {
      throw new InvalidInputError("Password doesn't meet complexity requirements") // Password doesn't meet complexity requirements
    }
