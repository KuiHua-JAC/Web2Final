...
try {
const { email, password, firstName, lastName, userName, isAdmin } = request.body;
let newUser = await userModel.addUser(email, password, firstName, lastName, userName, isAdmin);
response.status(200);
...
try {
if (await userModel.updateUserPassword(request.params.username, newPassword)) {
response.status(200);
response.send({
...
response.send({
  errorMessage:
    "Update user password failed. Unexpected error occurred " + err.message,
});
}
}
}
